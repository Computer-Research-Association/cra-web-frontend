...
import { Login, ReissueToken, ReqSignUp, ResTokenDto } from '~/models/Auth';
...
interface authStore {
  ...
  reissueToken: (_data: ReissueToken) => Promise<void>; // 저장된 Refresh Token으로 새로운 Access, Refresh Token
  // 받는 요청 처리
  ...
  refreshToken: string | null; // Access Token이 만료되었을때, 새로운 토근을
  // 발급받기 위한 Refresh Token을 저장
...
export const useAuthStore = create<authStore>()(
  // persist: 상태를 localStorage에 저장하고 브라우저를 새로고침해도 상태가
  // 유지되게 함
  persist(
    // 초기상태 설정
    (set) => ({
      ...
      login: async (data: Login) => {
        try {
          // 로그인 Api를 호출하여 사용자 인증을 처리하고, 서버로부터 중요한
          // 데이터를 반환
          ...
          // localStorage Storage에도 토큰을 저장하여 다른 Api 요청에서도
          // 사용할 수 있게하기
      ...
      signUp: async (data: ReqSignUp) => {
        try {
          // 새로운 사용자를 등록만 하기 때문에, 성공 or 실패 여부만 반환
          // (다른 반환값 굳이 처리할 필요 X And 별도의 상태 업데이트 필요
          // X)
      ...
      reissueToken: async (data: ReissueToken) => {
        try {
          // 토근 재발급 Api를 호출하여 새로운 ResponseToken(accessToken,
          // refreshToken, userId)을 받음
    ...
    // persist옵션에서 name을 지정하여 localStorage에 저장되는 key 이름을
    // 지정
    {
      name: 'auth-storage',
      storage: {
